<?php
	...
	if(isset($lecture_id)){
		$subjective_quest = "";
		$query = "SELECT 
			lsq.LectureSubjectiveQuestionId,lsq.SubjectiveQuestionId,lsq.Marks,lsq.WordLimit,sq.SubQuestionText,sq.explanation
			FROM `LectureSubjectiveQuestion` lsq 
			left join SubjectiveQuestion sq on sq.SubjectiveQuestionId = lsq.SubjectiveQuestionId
			where lsq.LectureId = '$lecture_id';";	
		
		$result = mysqli_query($conn,$query);
		$question = 0;
		while ($row = mysqli_fetch_array($result)) {
			$question++;
			$subjective_quest .= "<h2 class='et_question'><span style='display: table-cell;'>$question. </span><span style='display: table-cell; text-align:justify;'> $row[SubQuestionText]</span></h2>";
			$subjective_quest .= "<p class='et_question'><span style='display: table-cell;font-weight: 400;'>Marks: $row[Marks] </span> <span style='display: table-cell;font-weight: 400;'>&nbsp; Word Limit: $row[WordLimit]</span></p>";
		}
	}else{
		...
		<div class="container">
			<nav role="navigation">
				<ul class="pHeader_tabs">
					...
					<li class="active"><a href="assignment_dashboard.php?vid=<?php echo $lecture_id ?>&course_id=<?php echo $course_id ?>" >MAINS PRACTICE QUESTIONS</a></li>
				</ul>
			</nav>
		</div>
	</div>
